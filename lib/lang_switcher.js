...
function getLangAriaLabel(currentLang) {
    if (currentLang === 'es') return 'Seleccionar idioma';
    if (currentLang === 'nl') return 'Taal selecteren';
    return 'Select language';
}

function renderDesktopLangSelector(currentLang, urls) {
    const langUpper = currentLang.toUpperCase();
    const ariaLabel = getLangAriaLabel(currentLang);
    return `<div class="lang-selector">
                    <button class="lang-btn" aria-label="${ariaLabel}">
...
function renderMobileLangBar(currentLang, urls) {
    const label = currentLang === 'es' ? 'Idioma:' : (currentLang === 'nl' ? 'Taal:' : 'Language:');
    return `<div class="mobile-lang-bar">
                <span class="mobile-lang-label"><i class="fa-solid fa-globe"></i> ${label}</span>
